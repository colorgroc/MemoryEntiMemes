...
class SettingsScene: SKScene, ButtonDelegate {
    ...
    weak var settingsDelegate: SettingsSceneDelegate?
    ...
    override func didMove(to view: SKView) {
        ...
        backButton.position = CGPoint(x: (view.frame.width / 2.0) - (SettingsScene.buttonWidth / 2.0), y: 100)
        addChild(backButton)
    ...
    func onTap(sender: Button) {
        if sender == backButton {
            settingsDelegate?.back(sender: self)
        }
    }
